...
# 创建唯一索引
create unique index idx_unique_name on course(name);
# 创建联合索引
...
#1.选择工资在5000到12000的员工的姓名和工资   #between 相当于>= 而不是>
select first_name,salary from employees where salary between  5000 and 12000 order by salary;
#1.选择工资在5000到12000的员工的姓名和工资
select first_name,salary from employees where salary >= 5000 and salary <= 12000 order by salary;

# 2.选择在20或50号部门工作的员工姓名和部门号
select first_name,salary from employees  where department_id = 20 or department_id = 50;
...
# 6.选择姓名中有字母a和k的员工姓名 估计前后可能组合
select distinct first_name from employees where  first_name like '%a%%k%' or first_name like '%k%%a%';
...
select * from employees where employee_id in (80,90,100);
